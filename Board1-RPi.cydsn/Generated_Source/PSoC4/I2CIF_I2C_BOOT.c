...
static uint8 I2CIF_slReadBuf[I2CIF_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 I2CIF_slWriteBuf[I2CIF_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 I2CIF_applyBuffer;
...
static void I2CIF_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: I2CIF_I2CCyBtldrCommStart
********************************************************************************
*
* Summary:
...
{
    I2CIF_I2CSlaveInitWriteBuf(I2CIF_slWriteBuf, I2CIF_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    I2CIF_I2CSlaveInitReadBuf (I2CIF_slReadBuf, 0u);

    I2CIF_SetCustomInterruptHandler(&I2CIF_I2CResposeInsert);
    I2CIF_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: I2CIF_I2CCyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2CIF_I2CCyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2CIF_I2CCyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CIF_slWriteBuf - used to store received command.
...
cystatus I2CIF_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (I2CIF_I2C_SSTAT_WR_CMPLT & I2CIF_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) I2CIF_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) I2CIF_slWriteBuf,
                                              I2CIF_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(I2CIF_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: I2CIF_I2CCyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CIF_slReadBuf - used to store response.
*
*******************************************************************************/
cystatus I2CIF_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) I2CIF_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        I2CIF_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: I2CIF_I2CResposeInsert
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CIF_slRdBufSize - used to store slave read buffer size.
*  I2CIF_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  I2CIF_applyBuffer - the flag to release the buffer with
*  a response to read.
*
*******************************************************************************/
static void I2CIF_I2CResposeInsert(void)
{
    if (I2CIF_CHECK_INTR_SLAVE_MASKED(I2CIF_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (I2CIF_CHECK_I2C_STATUS(I2CIF_I2C_STATUS_S_READ))
        {
            ...
            if (0u != I2CIF_applyBuffer)
            {
                I2CIF_slRdBufSize  = I2CIF_applyBuffer;
                I2CIF_slRdBufIndex = 0u;
                I2CIF_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (I2CIF_slRdBufIndex != I2CIF_slRdBufSize)
            {
                I2CIF_slRdBufIndex = I2CIF_slRdBufSize;
            }
        }
    }
}
